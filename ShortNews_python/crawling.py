...
class UrlCrawling:
    ...
    def getEntertainmentUrl(self):
        today = str(datetime.datetime.now(timezone('Asia/Seoul')))[:11]  # 서울 기준 시간
    ...
    def getSportsUrl(self):      
        # today = str(datetime.datetime.now(timezone('Asia/Seoul')))[:11].replace('-', '')  # 서울 기준 시간
        ...
        for i in range(n[1]):  # 7
            url = f'https://sports.news.naver.com/{category[i]}/news/index?isphoto=N&page=1'
            ...
            soup = BeautifulSoup(browser.page_source, "html.parser")
            a_tag_list.extend(soup.select(".news_list li>a")[:10])
...
class ContentCrawling:
    ...
    def removeTag(self, content):
        ...
        try:
            while content[0].find("em"): content[0].find("em").decompose()
...
def startCrawling():
    ...
    duplication(url_crawler.url_df_list)    # 이미 요약한 기사 삭제
